function svgdraw() {
  ...
  //console.log(svg.childNodes.length);

  var combWidth = Math.min(bgWidth - SQRT3 * bgHeight / nDimmerHex,
    bgHeight / SQRT3 * (2 + 3 * nRings) / (1 + 2 * nRings)),
    combHeight = SQRT3 * (1 + 2 * nRings) * combWidth / (2 + 3 * nRings);

  var combHexWidth = 2 * combWidth / (2 + 3 * nRings),
    combHexHeight = SQRT3 * combHexWidth / 2;

  var dimmerHexHeight = combHeight / (nDimmerHex),
  ...
  for (col = 0; col <= 2 * nRings; col++) {
    ...
    for (row = 0; row <= nRings + rowOne; row++) {
